...
def load_adj(file_path: str, adj_type: str):
    ...
    # g_2 = np.dot(adj_mx, adj_mx)
    # g_3 = np.dot(np.dot(adj_mx, adj_mx), adj_mx)
    # g_2 = np.where(g_2 != 0, 1, 0)
    # g_3 = np.where(g_3 != 0, 1, 0)
    # adj_2 = [calculate_symmetric_normalized_laplacian(
    #     g_2).astype(np.float32).todense()]
    # adj_3 = [calculate_symmetric_normalized_laplacian(
    #     g_3).astype(np.float32).todense()]
