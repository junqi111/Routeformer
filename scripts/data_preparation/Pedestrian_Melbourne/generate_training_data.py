...
import pickle
import argparse
from dataset import NodeTrafficLoader,data_loader
from preprocess.GraphGenerator import GraphGenerator
import numpy as np
import torch
...
def generate_data(args: argparse.Namespace,data):
...
if __name__ == "__main__":
    ...
    DATASET_NAME = "Pedestrian_Melbourne"
    ...
    data_dir="datasets/raw_data/{0}/".format(DATASET_NAME)
    ...
    parser.add_argument("--dataset", default='Pedestrian', type=str, help="configuration file path")
    parser.add_argument("--city", default='Melbourne', type=str)
    ...
    uctb_data_loader = NodeTrafficLoader(dataset=args.dataset, city=args.city,
                                         ...
                                         MergeWay=args.MergeWay,data_dir=data_dir)
    data=uctb_data_loader.traffic_data
    ...
    # 指定要保存的文件路径
    output_file_path = os.path.join(OUTPUT_DIR, "adj_mx.pkl")
    # 使用pickle将列表保存为pkl文件
    with open(output_file_path, 'wb') as f:
        pickle.dump(supports, f)
    print(f"邻接矩阵已保存为 {output_file_path}")
    # print args
    print("-"*(20+45+5))
